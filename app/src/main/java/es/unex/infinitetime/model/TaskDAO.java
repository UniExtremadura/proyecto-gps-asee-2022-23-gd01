...
public interface TaskDAO {
    ...
    @Query("SELECT id, name, description, state, priority, effort, deadline, project_id, user_id, EXISTS(SELECT 1 FROM favorite WHERE task_id=id and user_id=:userId) AS is_favorite FROM task WHERE project_id=:projectId and state=:state ORDER BY priority DESC")
    LiveData<List<TaskWithFavorite>> getTasksProject(long projectId, long userId, int state);
